...
static void assert_receive_smtp_status(avs_net_abstract_socket_t *socket,
    ...
    while (1) {
        ...
        if (received >= strlen(line_full)
            ...
            if (memcmp(buffer + line_start,
                       line_beginning, strlen(line_beginning)) == 0
                    || memcmp(buffer + line_start,
                              line_full, strlen(line_full)) == 0) {
                break;
            }
        }
    }
...
AVS_UNIT_TEST(starttls, starttls_smtp) {
    ...
    AVS_UNIT_ASSERT_SUCCESS(avs_net_socket_send(socket,
                                                ehlo_msg, strlen(ehlo_msg)));
    assert_receive_smtp_status(socket, 250);
    AVS_UNIT_ASSERT_SUCCESS(avs_net_socket_send(socket, starttls_msg,
                                                strlen(starttls_msg)));
    assert_receive_smtp_status(socket, 220);

    memset(&ssl_config, 0, sizeof(ssl_config));
    ssl_config.version = AVS_NET_SSL_VERSION_TLSv1;
    AVS_UNIT_ASSERT_SUCCESS(avs_net_socket_decorate_in_place(&socket,
                                                             AVS_NET_SSL_SOCKET,
                                                             &ssl_config));
