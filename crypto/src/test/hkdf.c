...
AVS_UNIT_TEST(avs_crypto_hkdf, test_vector_1) {
    ...
    ASSERT_OK(avs_crypto_hkdf_sha_256(master_salt, sizeof(master_salt),
                                      master_secret, sizeof(master_secret),
                                      sender_key_info, sizeof(sender_key_info),
                                      output, &output_size));
    // Check of output_size intentionally omitted. Desired length of HKDF
    // output is encoded in info data and it's equal to sizes of expected
    // results.
    ASSERT_EQ_BYTES_SIZED(output, sender_key, sizeof(sender_key));

    memset(output, 0, sizeof(output));
    output_size = sizeof(output);
    ASSERT_OK(
        avs_crypto_hkdf_sha_256(master_salt, sizeof(master_salt),
                                master_secret, sizeof(master_secret),
                                recipient_key_info, sizeof(recipient_key_info),
                                output, &output_size));
    ASSERT_EQ_BYTES_SIZED(output, recipient_key, sizeof(recipient_key));
    ...
    ASSERT_OK(avs_crypto_hkdf_sha_256(master_salt, sizeof(master_salt),
                                      ...
                                      output, &output_size));
    ASSERT_EQ_BYTES_SIZED(output, common_iv, sizeof(common_iv));
}

AVS_UNIT_TEST(avs_crypto_hkdf, test_vector_2) {
    const unsigned char master_secret[] = {
        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
        0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10
    };
    ...
    memset(output, 0, sizeof(output));
    size_t output_size = sizeof(output);
    ASSERT_OK(avs_crypto_hkdf_sha_256(NULL, 0,
                                      master_secret, sizeof(master_secret),
                                      sender_key_info, sizeof(sender_key_info),
                                      output, &output_size));
    // Check of output_size intentionally omitted. Desired length of HKDF
    ...
    output_size = sizeof(output);
    ASSERT_OK(
        avs_crypto_hkdf_sha_256(NULL, 0,
                                master_secret, sizeof(master_secret),
                                recipient_key_info, sizeof(recipient_key_info),
                                output, &output_size));
    ASSERT_EQ_BYTES_SIZED(output, recipient_key, sizeof(recipient_key));
