...
AVS_UNIT_TEST(coap_msg, header_memory_layout) {
    avs_coap_msg_t *msg __attribute__((cleanup(free_msg))) =
            (avs_coap_msg_t *) avs_malloc(sizeof(*msg) + AVS_COAP_MSG_MIN_SIZE);
...
AVS_UNIT_TEST(coap_msg, header_fields) {
    avs_coap_msg_t *msg __attribute__((cleanup(free_msg))) =
            (avs_coap_msg_t *) avs_malloc(sizeof(*msg));
    setup_msg(msg, NULL, 0);

    AVS_UNIT_ASSERT_EQUAL(_avs_coap_header_get_version(msg), 1);
    AVS_UNIT_ASSERT_EQUAL(avs_coap_msg_get_type(msg),
...
AVS_UNIT_TEST(coap_msg, payload) {
    ...
    avs_coap_msg_t *msg __attribute__((cleanup(free_msg))) =
            (avs_coap_msg_t *) avs_malloc(sizeof(*msg) + sizeof(content) - 1);
    setup_msg(msg, content, sizeof(content) - 1);

    AVS_UNIT_ASSERT_NOT_NULL(avs_coap_msg_payload(msg));
    AVS_UNIT_ASSERT_EQUAL(avs_coap_msg_payload_length(msg),
                          sizeof(content) - 2);
    AVS_UNIT_ASSERT_EQUAL_BYTES_SIZED(avs_coap_msg_payload(msg), content + 1,
                                      sizeof(content) - 2);
}
...
#pragma GCC diagnostic pop
    ...
    avs_coap_msg_t *msg __attribute__((cleanup(free_msg))) =
            (avs_coap_msg_t *) avs_malloc(sizeof(*msg) + sizeof(content));
...
AVS_UNIT_TEST(coap_msg, validate_invalid_token_length) {
    ...
    AVS_FIELD_SET(msg->content[0], AVS_COAP_HEADER_TOKEN_LENGTH_MASK,
...
AVS_UNIT_TEST(coap_msg, validate_opt_length_overflow) {
    ...
    avs_coap_msg_t *msg __attribute__((cleanup(free_msg))) =
            (avs_coap_msg_t *) avs_malloc(sizeof(*msg) + sizeof(opts) - 1);
...
AVS_UNIT_TEST(coap_msg, validate_null_opt) {
    ...
    AVS_UNIT_ASSERT_TRUE(avs_coap_opt_value(optit.curr_opt)
                         == (const uint8_t *) optit.curr_opt + 1);
...
AVS_UNIT_TEST(coap_msg, validate_opt_ext_delta_byte) {
    ...
    avs_coap_opt_iterator_t optit = avs_coap_opt_begin(msg);
    AVS_UNIT_ASSERT_TRUE(optit.msg == msg);
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_number(&optit),
                          1 + AVS_COAP_EXT_U8_BASE);
    AVS_UNIT_ASSERT_NOT_NULL(optit.curr_opt);

    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_delta(optit.curr_opt),
                          1 + AVS_COAP_EXT_U8_BASE);
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_content_length(optit.curr_opt), 0);
    AVS_UNIT_ASSERT_TRUE(avs_coap_opt_value(optit.curr_opt)
                         == (const uint8_t *) optit.curr_opt + 2);
...
AVS_UNIT_TEST(coap_msg, validate_opt_ext_delta_short) {
    ...
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_content_length(optit.curr_opt), 0);
    AVS_UNIT_ASSERT_TRUE(avs_coap_opt_value(optit.curr_opt)
                         == (const uint8_t *) optit.curr_opt + 3);
...
#define OPTS_SIZE (2 + (1 + AVS_COAP_EXT_U8_BASE))
    ...
    avs_coap_msg_t *msg __attribute__((cleanup(free_msg))) =
            (avs_coap_msg_t *) avs_malloc(sizeof(*msg) + OPTS_SIZE);
...
#undef OPTS_SIZE
    ...
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_delta(optit.curr_opt), 0);
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_content_length(optit.curr_opt),
                          1 + AVS_COAP_EXT_U8_BASE);
    AVS_UNIT_ASSERT_TRUE(avs_coap_opt_value(optit.curr_opt)
                         == (const uint8_t *) optit.curr_opt + 2);
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_sizeof(optit.curr_opt),
                          1 + 1 + (1 + AVS_COAP_EXT_U8_BASE));
...
AVS_UNIT_TEST(coap_msg, validate_opt_ext_length_short) {
    ...
    avs_coap_msg_t *msg __attribute__((cleanup(free_msg))) =
            (avs_coap_msg_t *) avs_malloc(sizeof(*msg) + sizeof(opts)
    ...
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_delta(optit.curr_opt), 0);
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_content_length(optit.curr_opt),
                          256 + AVS_COAP_EXT_U16_BASE);
    AVS_UNIT_ASSERT_TRUE(avs_coap_opt_value(optit.curr_opt)
                         == (const uint8_t *) optit.curr_opt + 3);
    AVS_UNIT_ASSERT_EQUAL(avs_coap_opt_sizeof(optit.curr_opt),
                          1 + 2 + (256 + AVS_COAP_EXT_U16_BASE));
