...
AVS_UNIT_TEST(coap_msg, header_memory_layout) {
    avs_coap_msg_t *msg = (avs_coap_msg_t *)
	    &(uint8_t[sizeof(*msg) + AVS_COAP_MSG_MIN_SIZE]){0}[0];
...
AVS_UNIT_TEST(coap_msg, header_fields) {
    avs_coap_msg_t *msg = (avs_coap_msg_t *) &(uint8_t[sizeof(*msg)]){0}[0];
...
AVS_UNIT_TEST(coap_msg, payload) {
    ...
    avs_coap_msg_t *msg =
            (avs_coap_msg_t *) &(uint8_t[sizeof(*msg) + sizeof(content) - 1]){0}[0];
...
#pragma GCC diagnostic pop
    ...
    avs_coap_msg_t *msg =
            (avs_coap_msg_t *) &(uint8_t[sizeof(*msg) + sizeof(content)]){0}[0];
...
AVS_UNIT_TEST(coap_msg, validate_valid) {
    avs_coap_msg_t *msg =
            (avs_coap_msg_t *) &(uint8_t[sizeof(*msg)]){0}[0];
    setup_msg(msg, NULL, 0);
...
AVS_UNIT_TEST(coap_msg, validate_opt_length_overflow) {
    ...
    avs_coap_msg_t *msg =
            (avs_coap_msg_t *) &(uint8_t[sizeof(*msg) + sizeof(opts) - 1]){0}[0];
...
#define OPTS_SIZE (2 + (1 + AVS_COAP_EXT_U8_BASE))
    ...
    avs_coap_msg_t *msg = (avs_coap_msg_t *) &(uint8_t[sizeof(*msg) + OPTS_SIZE]){0}[0];
...
AVS_UNIT_TEST(coap_msg, validate_opt_ext_length_short) {
    ...
    avs_coap_msg_t *msg = (avs_coap_msg_t *) &(uint8_t[
            sizeof(*msg) + sizeof(opts) + (256 + AVS_COAP_EXT_U16_BASE) - 1]){0}[0];
...
#define DESERIALIZE_MSG(Content) \
    deserialize_msg(&(uint8_t[sizeof(uint32_t) + sizeof(Content) - 1]){0}[0], \
                    Content, sizeof(Content) - 1)

AVS_UNIT_TEST(coap_msg, fuzz_1_missing_token) {
    avs_coap_msg_t *msg = DESERIALIZE_MSG("\x68\x64\x05\x06\x0a");
    AVS_UNIT_ASSERT_FALSE(avs_coap_msg_is_valid(msg));
}

AVS_UNIT_TEST(coap_msg, fuzz_2_missing_option_ext_length) {
    avs_coap_msg_t *msg = DESERIALIZE_MSG("\x60\x64\x05\x06\xfa");
    AVS_UNIT_ASSERT_FALSE(avs_coap_msg_is_valid(msg));
}
