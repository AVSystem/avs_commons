...
AVS_UNIT_TEST(stream_membuf, peek) {
    ...
    AVS_UNIT_ASSERT_TRUE(
            avs_is_ok(avs_stream_peek(stream, strlen(str) - 1, &value)));
    AVS_UNIT_ASSERT_EQUAL(value, 'm');
    AVS_UNIT_ASSERT_TRUE(
            avs_is_ok(avs_stream_peek(stream, strlen(str), &value)));
...
AVS_UNIT_TEST(stream_membuf, reset) {
    ...
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_reset(stream)));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(
            avs_stream_read(stream, &bytes_read, &message_finished, buf, 4)));
    AVS_UNIT_ASSERT_EQUAL(bytes_read, 0);
    AVS_UNIT_ASSERT_EQUAL(message_finished, 1);
...
AVS_UNIT_TEST(stream_membuf, fit) {
    ...
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_membuf_fit(stream)));
...
AVS_UNIT_TEST(stream_membuf, defragment) {
    ...
    for (const char *ptr = str; *ptr; ++ptr) {
        AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_write(stream, ptr, 1)));
        char buf;
        size_t bytes_read;
        bool message_finished;
        AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_read(
...
AVS_UNIT_TEST(stream_getline, simple) {
    avs_stream_t *stream = avs_stream_membuf_create();
    AVS_UNIT_ASSERT_TRUE(
            avs_is_ok(avs_stream_write_f(stream, "HTTP/1.1 302 Found\r\n")));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(
            avs_stream_write_f(stream, "Cache-Control: private\r\n")));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_write_f(
    ...
    AVS_UNIT_ASSERT_TRUE(
            avs_is_ok(avs_stream_write_f(stream, "Content-Length: 79\r\n")));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_write_f(
            stream, "Date: Thu, 22 Dec 2016 17:13:21 GMT\r\n")));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_write_f(stream, "\r\n")));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_write_f(
...
AVS_UNIT_TEST(stream_getline, errors) {
    ...
    AVS_UNIT_ASSERT_FALSE(
            avs_is_ok(avs_stream_peekline(stream, 0, NULL, NULL, buf, 0)));
    AVS_UNIT_ASSERT_FALSE(
            avs_is_ok(avs_stream_peekline(stream, 0, NULL, NULL, NULL, 0)));
    ...
    buf[0] = -1;
    AVS_UNIT_ASSERT_FALSE(
            avs_is_ok(avs_stream_getline(stream, NULL, NULL, buf, 0)));
    AVS_UNIT_ASSERT_FALSE(
            avs_is_ok(avs_stream_getline(stream, NULL, NULL, NULL, 500)));
...
AVS_UNIT_TEST(stream_getline, exact) {
    ...
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_reset(stream)));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_write(
            stream, STREAM_DATA_CR_LF, strlen(STREAM_DATA_CR_LF))));
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_getline(
            stream, &bytes_read, &msg_finished, buf, sizeof(buf))));
    AVS_UNIT_ASSERT_EQUAL(bytes_read, 7);
    AVS_UNIT_ASSERT_EQUAL_STRING(buf, "1234567");
...
AVS_UNIT_TEST(stream_getline, inline_cr) {
    ...
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(last_err));
    AVS_UNIT_ASSERT_TRUE(msg_finished);
    AVS_UNIT_ASSERT_TRUE(avs_is_ok(avs_stream_cleanup(&stream)));
}
