...
static const char *const MONTY_PYTHON_RAW =
...
static void send_line(avs_stream_abstract_t *stream, const char **ptr) {
...
static const char *const MONTY_PYTHON_PER_LINE_REQUEST =
        "FDF\r\n"
        "A customer enters a pet shop.\n"
        "Customer: 'Ello, I wish to register a complaint.\n"
        "(The owner does not respond.)\n"
...
static const char *const MONTY_PYTHON_BIG_REQUEST =
        "1E\r\n"
        "A customer enters a pet shop.\n"
        "\r\n"
        "13E1\r\n"
...
AVS_UNIT_TEST(http, redirect) {
    ...
    avs_unit_mocksock_expect_connect(sockets[4], "isnickelbacktheworstbandever.tumblr.com", "80");
    tmp_data =
            "GET / HTTP/1.1\r\n"
    ...
    AVS_UNIT_ASSERT_FAILED(avs_stream_finish_message(stream));
    AVS_UNIT_ASSERT_EQUAL(avs_stream_errno(stream),
...
AVS_UNIT_TEST(http, interleaving) {
    ...
    avs_unit_mocksock_input(socket, tmp_data, strlen(tmp_data));
    AVS_UNIT_ASSERT_SUCCESS(avs_stream_finish_message(stream));
    AVS_UNIT_ASSERT_SUCCESS(avs_stream_getline(stream, NULL,
                                               &message_finished,
    ...
    while (!message_finished) {
        int result = avs_stream_getline(stream, NULL, &message_finished,
                                        buffer, sizeof(buffer));
        AVS_UNIT_ASSERT_TRUE(!result || message_finished);
        if (message_finished) {
            break;
        }
        for (i = 0; buffer[i]; ++i) {
...
AVS_UNIT_TEST(http, interleaving_error) {
    ...
    tmp_data =
            ...
            "'and' and the Little Blind Text should turn around and return to\n"
            "its own, safe country. But nothing the copy said could convince\n"
            "her and so it didn't take long until a few insidious Copy\n"
            ...
            "projects again and again. And if she hasn't been rewritten,\n"
            "then they are still using her. Far far away, behind the word\n"
            ...
            "of blind text by the name of Lorem Ipsum decided to leave for\n"
            "the far World of Grammar. The Big Oxmox advised her not to do\n"
            "so, because there were thousands of bad Commas, wild Question\n"
            "Marks and devious Semikoli, but the Little Blind Text didn't\n"
            "listen. She packed her seven versalia, put her initial into the\n"
            "belt and made herself on the way. When she reached the first\n"
            "hills of the Italic Mountains, she had a last view back on the\n"
            "skyline of her hometown Bookmarksgrove, the headline of Alphabet\n"
            "Village and the subline of her own road, the Line Lane. Pityful\n"
            "a rethoric question ran over her cheek, then she continued her\n"
            "way. On her way she met a copy. The copy warned the Little Blind\n"
...
AVS_UNIT_TEST(http, send_headers_fail) {
    ...
    avs_unit_mocksock_expect_errno(socket, EIO);
    AVS_UNIT_ASSERT_FAILED(avs_stream_finish_message(stream));
    avs_unit_mocksock_assert_io_clean(socket);
    avs_unit_mocksock_expect_shutdown(socket);
    avs_stream_cleanup(&stream);
