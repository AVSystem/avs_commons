...
append_data(avs_coap_msg_buffer_t *buffer, const void *data, size_t data_size) {
    if (data_size > bytes_remaining(buffer)) {
        LOG(ERROR,
            _("cannot append ") "%u" _(" bytes, only ") "%u" _(" available"),
...
static int append_option(avs_coap_msg_buffer_t *buffer,
    ...
    buffer->msg->length +=
            (uint32_t) opt_write_header(msg_end_ptr(buffer), opt_number_delta,
                                        opt_data_size);
...
int avs_coap_msg_builder_reset(avs_coap_msg_builder_t *builder,
    ...
    if (builder->msg_buffer.capacity
        ...
        LOG(ERROR,
            _("message buffer too small: ") "%u" _("/") "%u" _(" B available"),
